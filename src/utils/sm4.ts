...
export function SM4Util() {
  // 和后端key一致
  const arr = 'qaqGJwsXX_BzW=gJWJW'; // 防止完整密钥出现在代码里
  const newarr = arr.substring(3);
  this.secretKey = newarr;
  // 当时用CBC模式的时候
  const str = 'ZkR_SiNoSOFT=568yyy';
  const newStr = str.substring(0, 16);
  this.iv = newStr;
